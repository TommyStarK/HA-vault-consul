...
# curl --header "X-Vault-Token: s.O5cZZlMnocsGq4141oHTl548" --request POST  --data "$(pki_engine_config)" http://127.0.0.1:8201/v1/sys/mounts/pki | jq
...
# curl --header "X-Vault-Token: s.O5cZZlMnocsGq4141oHTl548" --request POST  --data "$(root_certificate_authority_config)" http://127.0.0.1:8201/v1/pki/root/generate/internal | jq -r ".data.certificate" > CA_cert.crt
curl --header "X-Vault-Token: $ROOT_TOKEN"  \
    --request POST \
...
# curl --header "X-Vault-Token: s.O5cZZlMnocsGq4141oHTl548" --request POST  --data "$(root_ca_crl_urls)" http://127.0.0.1:8201/v1/pki/config/urls | jq
curl --header "X-Vault-Token: $ROOT_TOKEN"  \
    --request POST \
